...
> 用于手动逐项检查当前实现情况。当前基线：新建场景默认 `sceneSize=15x15`、`outerPadding=1`、`canvasSize=17x17`；用户可在场景控制区选择 `6..20` 的画布宽高；legacy 5x5/7x7 场景仍按自身尺寸恢复。

## 工作台与基础布局

- [*] 首屏是工作台，不是 landing page。
- [*] 顶部工具栏、场景/Pokemon 摘要、建筑层面板、中央 17x17 默认画布、右侧素材栏、底部检查器快捷栏和独立预览/导出入口共同组成主界面。
...
- [ ] Desktop 编辑工作台不常驻显示旧预览面板。
- [ ] 用户可以通过顶部预览/导出入口打开独立预览模式。
- [ ] 独立预览展示整体素材清单、逐层图形、逐层素材清单和层备注。
- [ ] 独立预览支持下载整体图片。
- [ ] 独立预览支持按层下载图片。
- [ ] 独立预览使用完整当前尺寸画布：默认 17x17，legacy 7x7。
- [ ] 独立预览中的逐层图形展示当前主体区、外围装饰区、footprint 和 stacking 表达。
- [ ] 预览模式不改变 SceneDocument payload。
- [ ] 预览模式不触发 autosave，不保存 export summary。
- [ ] 预览模式打开/关闭后工作台格子尺寸不明显跳动。

## 下层影子辅助模式

- [ ] 编辑 0 层时不显示下层影子。
- [ ] 编辑 L1 时可以看到 L0 的半透明素材影子；编辑 L2 时可以看到 L1 的半透明素材影子。
- [ ] 下层影子只显示直接下一层，不显示所有低层。
- [ ] 下层影子按 lower layer 的 footprint、rotation 和 dye 渲染。
- [ ] 下层影子不显示技能标记、备注、选择框、删除按钮或旋转按钮。
- [ ] 下层影子不可选中、不可删除、不可旋转、不可触发检查器。
- [ ] 点击下层影子所在格仍按当前层选择/放置逻辑执行。
- [ ] 下层影子不参与 occupancy、stacking、replacement confirmation、footprint conflict 或 height blocking。
- [ ] 下层影子开关只写 UI preferences/localStorage，不进入 SceneDocument、PSE 字符串、export payload 或 autosave payload。
...
- 发布前必须确认默认目标为 `sceneSize=15x15`、`outerPadding=1`、`canvasSize=17x17`，并确认自定义画布宽高 `6..20` 仍能保存、恢复、导出。
