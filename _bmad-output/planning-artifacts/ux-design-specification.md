...
pokopia-scene-editor 是一个面向 Pokopia 布景创作者的结构化 5×5 布景编辑器。产品用 7×7 实际编辑画布表达中心 5×5 主体区和外围一圈装饰区，让用户可以在同一数据模型中完成素材摆放、建筑层管理、百变怪技能标记、俯视图/正视图预览，以及保存和导出。
...
- 建筑层、锁定/隐藏状态、当前编辑层、技能标记和选中格状态必须清楚可见，不能只依赖颜色。
- 俯视图和正视图需要足够直观，但 MVP 不应陷入复杂真实视角或遮挡模拟。

### Design Opportunities

- 通过稳定的三栏/多区域编辑布局，把素材、画布、属性和建筑层组织成高效的创作工作台。
- 通过主体区边界、外围区样式、技能角标、层级状态和预览切换，让 Pokopia 的布景规则自然可见。
- 通过 Open Design 原型流程生成更贴近真实工具界面的设计方向，而不是输出营销页式 UI。
- 通过清晰的数据恢复、错误提示和导出结构，让用户信任方案可以被复现。

## Core User Experience

### Defining Experience

核心体验是：用户在规则始终可见的 7×7 多建筑层画布中，通过搜索和筛选选择素材，在当前建筑层放置一个素材实例，为该实例标记百变怪技能，并通过即时画布反馈、结构预览和可信 JSON 状态，持续确认这个布景可以被保存、导出并忠实重建。

该编辑器不是通用绘图工具。每次放置都必须明确绑定到一个具体实例：素材 ID、x/y 坐标、主体区或外围装饰区、建筑层、朝向、百变怪技能标记和备注。用户应始终知道自己正在编辑的是哪个建筑层、哪个格子、哪个素材实例。

### Platform Strategy

MVP 采用桌面优先 Web App。主要交互面向鼠标和键盘，支持高频搜索、筛选、悬停预览、点击放置、实例选中、属性编辑、建筑层切换和预览切换。

移动端不作为完整编辑主场景，但窄视口下必须保证页面不崩溃、关键状态可访问、控件不重叠。离线能力不是 MVP 核心要求，但本地 JSON 保存、导出和重新打开必须可靠。

### Effortless Interactions

画布始终显示完整 7×7 编辑范围，其中中心 5×5 是主体区，外围一圈是装饰区。主体区和外围区必须在边界、背景、悬停状态、选中状态、坐标提示和预览中保持一致区分。

用户选择素材后，画布进入明确的待放置状态。悬停格子时应显示放置预览、目标坐标、区域类型、当前建筑层、技能状态和是否可放置。点击后，素材成为可选中的实例，用户可以立即在属性面板中确认或修改坐标、层级、朝向、技能标记和备注。

建筑层是一等放置上下文，不是附属选项。同一 x/y 坐标可以在不同建筑层上存在不同素材；切换建筑层时，界面必须避免用户把隐藏层内容误认为丢失或被覆盖。
...
第一次实例修改：用户点击已放置素材后，可以直接修改该实例的朝向、技能标记、备注或建筑层，不需要重新放置。

第一次可信导出：用户导出 JSON 并重新打开后，画布、建筑层、素材 ID、坐标、主体/外围区分、技能标记和预览结果完整还原；无法恢复的数据必须明确提示，不能静默降级。

### Experience Principles

- 规则可见：5×5 主体区、7×7 实际画布、外围装饰区和建筑层关系必须持续可见。
- 实例优先：用户编辑的是放置实例，而不是抽象素材或普通格子。
- 上下文不隐藏：当前素材、当前建筑层、悬停/选中格、区域类型和技能状态必须清楚表达。
- 错误先预防：放置、替换、删除和修改技能状态都应有即时反馈；覆盖已有实例前必须让用户看到被影响对象。
- 预览用于校验：俯视图确认 7×7 平面布局，正视图确认建筑层高度关系；MVP 预览优先服务排错和复现，不追求复杂真实视角模拟。
- 数据可信：每个可见放置都能映射回明确 JSON 数据，每个 JSON 状态都能被视觉重建。
...
完成布景时，用户应感到可复现和可托付：保存、导出和重新打开都强化“这个方案可以继续编辑，也可以交给别人理解”。

出错时，用户应感到可恢复，而不是焦虑。导入失败、覆盖风险、字段缺失、锁定层编辑等问题应给出具体原因和下一步修复方向。

### Micro-Emotions

- 信心，而不是困惑：用户始终知道当前素材、当前建筑层、当前格子和技能状态。
- 信任，而不是怀疑：画布、属性面板、预览和 JSON 状态保持一致。
...
信任来自一致的数据表达：每个可见素材都能在属性面板和导出数据中找到对应字段，预览也必须从同一场景状态派生。

专注来自工具式布局：素材、画布、属性、建筑层和预览应形成稳定工作台，避免营销页式叙事、装饰性卡片和不必要的大面积说明。

可恢复感来自错误预防：覆盖、删除、导入失败、锁定层操作和无法还原的数据都应在用户损失前给出明确反馈。
...
- 常用工具固定可达：保存、导出、撤销、删除、旋转、俯视图、正视图、网格、主体边界和技能标记等高频操作应在顶部工具栏或紧邻画布的位置可达。
- 上下文操作浮现：选中已放置素材实例后，显示与该实例相关的快捷操作，例如旋转、删除、技能标记、移动层级和查看属性。
- 网格与对齐反馈：画布应持续提供稳定格线、悬停格、选中格、放置预览和区域身份反馈，让用户不需要猜测素材落点。
- 低饱和语义视觉：主体区、外围区、锁定层、隐藏层和技能标记可以使用克制的色彩、边框、角标和透明度组合表达，避免装饰性视觉干扰结构判断。
...
- Component set：工具栏按钮、素材卡片、筛选控件、建筑层行、属性字段、预览切换、状态角标、上下文操作条、导入/导出错误提示。
...
如果这个体验做对，用户会用一句话描述产品：“我可以像搭建 Pokopia 布景一样，把每个素材按格子和层级放进去，并且导出后还能完整还原。”
...
**Interaction:** 用户在 7×7 画布上悬停目标格。系统显示放置预览、坐标、主体/外围区域、当前建筑层、是否可放置以及可能覆盖的已有实例。用户点击后完成放置。

**Feedback:** 放置完成后，目标格出现素材实例、选中状态和必要的技能标记。属性面板同步显示素材 ID、坐标、区域类型、建筑层、朝向、技能状态和备注。若操作会失败或覆盖内容，系统在执行前给出明确反馈。

**Completion:** 用户可以通过俯视图和正视图验证布景结构，也可以保存或导出 JSON。完成状态不是“画面看起来对”，而是画布、属性面板、预览和导出数据都指向同一场景状态。

**Recovery:** 用户可以通过选中实例修改技能状态、朝向、备注或建筑层；通过隐藏/显示/锁定建筑层排查结构；通过导入校验和错误提示修复无法恢复的数据。
...
- Mono / Data：SF Mono、Consolas 或等价 monospace，用于坐标、层号、素材 ID、颜色值、JSON 状态、计数和短标签。
...
- 顶部：保存、导出、撤销、重做、删除、旋转、网格、主体边界、技能标记显示等高频工具。
...
这些方向分别测试了三栏工作台、高频工具轨、预览优先、建筑层审计、宝可梦主色动态背景和 JSON 数据可信度等设计重点。

### Chosen Direction

选择 Direction A：Decor Dex Workbench。

该方向采用最接近 Pokopia Decor Dex 的三栏工作台结构：左侧素材与搜索，中央 7×7 编辑画布，右侧选中实例属性和预览状态。顶部工具栏承载保存、导出、撤销、重做、网格、主体边界和技能标记等高频操作。
...
- 顶部栏：保存、导出、撤销、重做、删除、旋转、网格、主体边界、技能标记显示等高频工具。
...
用户从空白场景开始，在 Direction A 的三栏工作台中完成一个可保存、可导出的 7×7 多建筑层布景。该流程覆盖素材搜索、建筑层选择、放置预览、实例属性确认、俯视/正视预览和 JSON 导出。
...
flowchart TD
  ...
  G -- "否" --> I["显示空状态或建议调整筛选"]
  I --> F
  H --> J["素材信息随放置实例进入属性面板和导出数据"]
```

### 导出数据无法复现时的排查

用户重新打开或导入 JSON 时，如果数据缺失、类型错误或坐标越界，系统应阻止静默成功，并提供字段路径、原因和修复方向。

```mermaid
flowchart TD
  A["用户导入或重新打开 JSON"] --> B["系统校验 sceneSize、canvasSize、outerPadding"]
...
- 危险操作先解释：删除建筑层、覆盖实例、导入失败和无法恢复的数据必须明确提示影响范围。
- 先显示再执行：悬停阶段尽量暴露目标坐标、区域、建筑层和覆盖风险。
- 面板服务当前对象：左侧服务素材选择，中央服务画布编辑，右侧服务选中实例和数据确认。
- 保存导出是信任闭环：导出前后都应让用户知道当前数据是否有效、是否有未保存修改。
...
- 顶部工具栏按钮：保存、导出、撤销、重做、删除、旋转、显示网格、显示主体边界、显示技能标记。
- 搜索输入与筛选按钮：用于素材搜索、分类筛选、区域筛选和技能筛选。
- 纸面面板：左侧素材栏、右侧属性面板、建筑层面板和导入/导出反馈。
- 表单字段：文本输入、选择器、开关、复选框、备注输入。
- 状态标签：当前建筑层、当前素材、区域类型、技能状态、未保存状态和校验状态。
- 确认与错误提示：删除建筑层、覆盖实例、导入失败和导出校验失败。
...
**Anatomy:** 7×7 网格、主体区边界、外围区背景、格子坐标、放置预览、选中格、技能角标、锁定层视觉。
**States:** 默认、悬停、待放置、已选中、不可放置、将覆盖、锁定层、隐藏层参与预览。
**Accessibility:** 提供画布整体标签、当前选中格说明、键盘方向移动支持和每格区域/坐标/素材状态说明。
**Interaction Behavior:** 悬停显示坐标、区域、建筑层和放置合法性；点击放置或选中实例；冲突操作先提示。

#### Asset Picker

**Purpose:** 帮助用户快速找到并选择当前要放置的素材。
**Usage:** Direction A 左侧栏。
**Anatomy:** 搜索框、结果计数、分类筛选、区域筛选、技能筛选、素材卡片、素材缩略图、名称、分类、标签、默认技能状态。
...
**Anatomy:** 素材 ID、素材名称、坐标、区域类型、建筑层、朝向、技能标记、技能类型、技能备注、格子备注、数据状态。
...
**Usage:** 画布周边或右侧结构预览区。
**Anatomy:** 视图切换、显示网格、显示主体边界、显示技能标记、当前层/全部可见层选项。
**States:** 俯视图、正视图、当前层预览、全部可见层预览、隐藏层排除、技能标记隐藏。
**Accessibility:** 使用分段控件或标签页语义；状态切换有可访问名称。
**Interaction Behavior:** 预览从同一场景状态派生，不允许形成独立编辑状态。
...
**Accessibility:** 错误摘要可被屏幕阅读器读取；错误列表可键盘访问。
**Interaction Behavior:** 校验失败时不覆盖当前场景；成功恢复后同步画布、属性、建筑层和预览状态。

### Component Implementation Strategy

先实现状态与布局核心组件，再补齐增强组件。所有组件使用共享 tokens：动态宝可梦主题 token、纸面面板 token、语义状态 token、网格尺寸 token 和 typography token。

自定义组件必须以场景数据为唯一来源。画布、属性面板、建筑层列表、预览和导出校验不能维护互相分叉的独立状态。
...
主操作按钮只用于会推进场景状态的关键动作，例如保存、导出、确认导入、确认删除建筑层。主操作在同一上下文中最多出现一个。

次级按钮用于可逆或辅助操作，例如取消、重试、显示全部、切换预览。图标按钮用于高频工具栏动作，例如撤销、重做、删除、旋转、显示网格、显示主体边界、显示技能标记；图标按钮必须提供可访问名称和悬停说明。

危险操作使用明确的破坏性样式和确认流程。删除建筑层、覆盖已有实例、导入会替换当前场景等操作必须说明影响范围。

### Feedback Patterns

即时反馈优先于事后错误。悬停格子时应先显示目标坐标、区域、当前建筑层、放置合法性和覆盖风险。

成功反馈应轻量，不打断编辑流。例如放置成功后保持实例选中并更新属性面板；保存成功后更新未保存状态；导出成功后显示文件状态或短暂提示。

错误反馈必须包含：问题对象、字段路径或操作原因、实际问题、用户可执行的修复方向。导入和恢复失败不得静默降级。

警告用于可能导致数据变化但尚未执行的操作，例如覆盖实例、删除非空建筑层、切换到锁定层编辑。信息提示用于说明隐藏层、锁定层、当前预览范围等上下文。
...
字段校验就地显示，错误信息贴近字段，同时在导入/导出校验场景中汇总到错误摘要。任何字段错误都不能只靠红色表达。
...
弹窗只用于必须暂停用户的场景：删除非空建筑层、导入替换当前场景、导出/导入结构错误详情。普通属性编辑、技能切换、旋转和预览切换不使用弹窗。
...
恢复状态必须保护当前数据。导入失败、重新打开失败或 JSON 校验失败时，不覆盖当前场景，并提供重试、取消和查看错误详情。
...
画布格子状态采用固定语义：主体区、外围区、悬停、选中、放置预览、不可放置、技能标记、锁定层内容均有独立组合样式。
...
`<768px` 进入 **Mobile View-only Mode**。Mobile 不是简化编辑器，而是只读检查器/预览器。允许查看场景、切换楼层、点选格子或实例查看属性、查看素材信息、缩放和平移画布、查看导入校验结果。禁止放置、移动、旋转、删除、修改属性、修改楼层、切换技能状态、保存、自动保存、导入覆盖、撤销/重做、批量清空和任何会改变场景数据或 dirty state 的行为。
...
- 保存、自动保存、导入覆盖、撤销/重做、删除快捷键、保存快捷键都不能改变 scene JSON。
- 执行所有 Mobile 只读交互后，比对 scene JSON 快照必须完全一致。
- 导入校验可以显示错误和定位，但不能自动修复或写回场景。
...
- 导入 JSON 中的名称、备注、技能说明必须按文本安全渲染，不能作为 HTML 注入。
