...
pokopia-scene-editor 是一个面向 Pokopia 布景创作者的结构化布景编辑器。产品默认用 17×17 实际编辑画布表达中心 15×15 主体区和外围一圈装饰区，让用户可以在同一数据模型中完成素材摆放、真实 footprint 占用、建筑层管理、建筑层备注、百变怪技能标记、俯视图/正视图预览，以及保存和导出。旧 7×7 场景仍按自身尺寸恢复和查看。
...
本 UX 规格已按 `_bmad-output/planning-artifacts/sprint-change-proposal-2026-05-28.md` 增加 Epic 10 建筑层备注体验。层备注是按建筑层填写和列出的多条用户备注，不是普通素材实例备注，也不是 UI preference。

选中当前建筑层的空格时，备注输入框显示在选中空格提示框下方，并列出当前层已有备注。图片导出预览中，每层备注显示在该层素材清单下方。备注正文保持用户原文，不随语言切换自动翻译，并且在工作台和导出预览中都只能作为安全文本渲染。

### Approved Course Correction - 2026-05-28 承载面/叠放规则

本 UX 规格已按 `_bmad-output/planning-artifacts/sprint-change-proposal-2026-05-28-stacking-surface-rules.md` 增加承载面与受控叠放规则。终端用户工作台必须能表达“可以放到盘子上的食物”“可以压在地毯/底垫/嫩芽/低高度素材上的兼容物品”和“不兼容叠放”三种状态，而不是把所有同层 overlap 都显示成相同的 footprint 冲突。合法同格叠放的显示方案是把原始格子或对应 footprint cell 拆分成上下两部分：下半部分显示可承载/可叠放的 base 素材，上半部分显示被承载/叠放的 top 素材。

本次 UX 不新增单独 stacking editor、手动 z-index 控制、父子绑定控件或 SceneDocument v2。素材维护者通过 catalog metadata 维护 surface rules；创作者仍通过普通选择素材、悬停格子、点击放置、选中实例和查看预览完成操作。承载/被承载关系是 `scene-core` 派生结果，不是用户可编辑或可保存字段。

### Approved Course Correction - 2026-05-29 15x15 Scene Size / 17x17 编辑画布

本 UX 规格已按 `_bmad-output/planning-artifacts/sprint-change-proposal-2026-05-29-15x15-scene-size.md` 增加 Epic 12 尺寸扩展体验。新建场景默认是 `sceneSize=15x15`、`outerPadding=1`、`canvasSize=17x17`；旧 7×7 场景仍按自身尺寸恢复。所有画布、预览、导出和可访问标签必须使用 scene dimensions，而不是写死 7×7。
...
画布始终基于当前 SceneDocument dimensions 显示完整编辑范围；新建场景默认完整范围为 17×17，其中中心 15×15 是主体区，外围一圈是装饰区。主体区和外围区必须在边界、背景、悬停状态、选中状态、坐标提示和预览中保持一致区分。

用户选择素材后，画布进入明确的待放置状态。悬停格子时应显示放置预览、目标 anchor 坐标、区域类型、当前建筑层、effective footprint、技能状态、承载/叠放状态和放置合法性。点击后，素材成为可选中的实例，用户可以立即在属性面板中确认或修改朝向、染色、技能标记和技能备注。

建筑层是一等放置上下文，不是附属选项。同一 x/y 坐标可以在不同建筑层上存在不同素材；同一建筑层同坐标默认不能叠放，只有 catalog 明确允许的承载面或底垫/低高度素材可以形成合法 overlap。如果下层素材 height 大于 1，上方建筑层的对应 footprint cells 必须显示为不可放置；stacking rule 不得让用户误以为可以绕过 height 阻塞。切换建筑层时，界面必须避免用户误以为其他层同坐标内容被覆盖，也不能让派生阻塞或承载关系看起来像真实保存的素材实例字段。
...
第一次理解承载面：用户选择食物并悬停到盘子、木盘子或派对拼盘上，看到可承载预览；选择非食物悬停到同一位置时，看到不兼容叠放原因。
...
- 规则可见：5×5 主体区、7×7 实际画布、外围装饰区、素材 footprint、承载/叠放状态和建筑层关系必须持续可见。
- 实例优先：用户编辑的是放置实例，而不是抽象素材或普通格子。
- 上下文不隐藏：当前素材、当前建筑层、悬停/选中格、区域类型和技能状态必须清楚表达。
- 错误先预防：放置、替换、删除和修改技能状态都应有即时反馈；覆盖已有实例、越界、被下层 height 阻塞或不兼容叠放前必须让用户看到被影响对象和原因。
...
- 避免工具面板压过画布：素材库和属性字段不能让中心尺寸驱动画布失去主导地位；默认 17×17 场景必须保留可读坐标和稳定格子比例。
...
- 避免无限画布心智误导：Whimsical 的无限画布适合白板，但本产品必须强调固定尺寸规则画布；新建场景默认 17×17，不能让用户以为可以任意扩展编辑范围。
...
轻量自有设计系统更适合本项目，因为它可以围绕 Whimsical 式工作台布局、尺寸驱动规则画布、低饱和语义视觉和实例级属性编辑建立专用组件，而不是被通用组件库的信息架构牵引。
...
定义性体验是“在尺寸驱动多建筑层画布上放置并验证一个可复现的布景实例”；新建场景默认 17×17，中心 15×15 是主体区。
...
需要特别设计的是这些模式的组合方式：尺寸驱动规则画布与多建筑层叠放必须同时可理解；百变怪技能标记必须以实例属性表达；外围装饰区必须被视为真实可编辑区域，而不是画布边框或装饰背景。
...
**Interaction:** 用户在 7×7 画布上悬停目标格。系统显示放置预览、anchor 坐标、主体/外围区域、当前建筑层、effective footprint、是否可放置、可能覆盖或承载的已有实例、承载/叠放兼容性以及跨层阻塞来源。用户点击后完成放置。

**Feedback:** 放置完成后，目标 anchor 与 footprint 覆盖范围出现同一个素材实例、选中状态和必要的技能标记。属性面板同步显示素材 ID、anchor 坐标、区域类型、建筑层、朝向、footprint、承载/被承载状态、技能状态、技能备注和当前层备注。若操作会失败、越界、覆盖内容、不兼容叠放或被下层 height 阻塞，系统在执行前给出明确反馈。
...
- 右侧：选中实例属性、坐标、区域、建筑层、朝向、技能标记、技能备注、当前层备注和数据状态。
...
该方向采用最接近 Pokopia Decor Dex 的三栏工作台结构：左侧素材与搜索，中央尺寸驱动编辑画布，右侧选中实例属性和预览状态。顶部工具栏承载图片导出、删除、旋转、染色和预览相关高频操作；新建场景默认 17×17。
...
- 右侧栏：选中实例属性、坐标、区域、建筑层、朝向、技能标记、技能备注、当前层备注、结构预览。
...
用户从空白场景开始，在 Direction A 的三栏工作台中完成一个可保存、可导出图片的 7×7 多建筑层布景。该流程覆盖素材搜索、建筑层选择、层备注填写、放置预览、实例属性确认、俯视/正视预览和图片导出。
...
flowchart TD
  ...
  C --> D["系统校验坐标、区域、层号、层备注、素材 ID 和技能字段"]
  D --> E{"校验通过？"}
...
**Accessibility:** 层级列表支持键盘导航；拖动 handle 和上移/下移控件有明确可访问名称，并通过 live announcement 反馈排序结果。
**Interaction Behavior:** 切换层级不清空选中上下文；删除层必须提示影响实例数量。用户拖动层行时列表即时显示目标顺序；取消拖动恢复原顺序。Drop 后提交排序并触发现有自动保存。键盘用户可通过上移/下移控件完成同一排序结果。

#### Instance Inspector

**Purpose:** 编辑当前选中素材实例的完整属性，并在选中空格时维护当前建筑层备注。
**Usage:** Direction A 右侧栏。
**Anatomy:** 素材 ID、素材名称、anchor 坐标、区域类型、建筑层、朝向、footprint/effective footprint、承载/被承载关系、技能标记、技能类型、技能备注、当前层备注输入、当前层备注列表、数据状态。
**States:** 未选中、选中空格、选中素材实例、锁定层只读、字段错误、未保存修改。
**Accessibility:** 字段标签清楚；只读状态和错误状态不能只靠颜色；表单控件支持键盘操作。
**Interaction Behavior:** 修改字段后立即反映到画布、预览和导出状态；技能标记只作用于当前实例。选中当前建筑层的空格时，选中空格提示框下方显示层备注输入、添加动作和备注列表；新增、编辑、删除备注只作用于当前建筑层，不作用于当前格子或素材实例。选中素材实例时，不把层备注误显示为素材实例字段。

#### Preview Switcher

**Purpose:** 在俯视图和正视图之间切换，帮助用户校验结构。
**Usage:** 画布周边或右侧结构预览区。
**Anatomy:** 视图切换、当前层/全部层选项、跨格素材、承载/被承载关系、height 高度表达、不可放置来源提示。
**States:** 俯视图、正视图、当前层预览、全部可见层预览、隐藏层排除、技能标记隐藏。
**Accessibility:** 使用分段控件或标签页语义；状态切换有可访问名称。
**Interaction Behavior:** 预览从同一场景状态、asset catalog footprint 和 stacking metadata 派生，不允许形成独立编辑状态；派生 blocking cells 和 stacking relations 只用于解释不可放置或承载关系，不计入真实素材数量。
...
**Anatomy:** 图片标题区、整体素材清单、逐层图形、跨格 footprint 表达、承载/叠放关系表达、逐层素材清单、逐层备注、下载图片、关闭、生成失败提示。
**States:** 预览生成中、预览有效、预览失败、下载成功、下载失败、空层。
**Accessibility:** 下载图片、关闭和失败提示必须有可访问名称；素材清单和层标题应有可读文本结构。
**Interaction Behavior:** 预览和下载从当前 scene、asset catalog footprint 与 stacking metadata 派生，不覆盖当前场景，不触发 autosave，不写入 saved storage 或 UI preferences。每个建筑层的备注区域显示在该层素材清单下方，备注标题使用系统语言，备注正文保持用户原文。无备注层可以隐藏备注区域，避免导出图片产生空占位。后端 Worker 可以为 API/MCP/Codex 返回同语义的 export summary JSON，但第一阶段不替代浏览器内图片预览和下载。
...
即时反馈优先于事后错误。悬停格子时应先显示目标坐标、区域、当前建筑层、effective footprint、承载/叠放兼容性、放置合法性、越界风险、覆盖风险和跨层阻塞来源。

成功反馈应轻量，不打断编辑流。例如放置成功后保持实例选中并更新属性面板；图片导出成功后显示文件状态或短暂提示。

错误反馈必须包含：问题对象、字段路径或操作原因、实际问题、用户可执行的修复方向。Footprint 冲突还必须说明触发素材、阻塞素材、建筑层和坐标集合；stacking 冲突还必须说明 top 素材、base 素材、surface 类型、建筑层和坐标集合。不兼容叠放在画布上使用浅红底、红色边框或状态标签，并配合文字原因，视觉强度和交互时机应与 Epic 8 的跨层 height 阻塞提示保持一致。图片导出失败和恢复失败不得静默降级；导入仍是 Post-MVP。

警告用于可能导致数据变化但尚未执行的操作，例如覆盖实例、删除非空建筑层、跨格 footprint 覆盖已有内容或不兼容叠放。信息提示用于说明当前预览范围、height 派生阻塞、合法承载/被承载关系和不可放置来源等上下文。

### Form Patterns

属性面板字段按“实例身份 -> 位置 -> 建筑层 -> 朝向 -> 技能 -> 当前层备注”的顺序排列。用户应先确认自己在编辑什么，再修改细节。

只读字段用于显示素材 ID、坐标、区域类型等系统派生信息；可编辑字段用于朝向、建筑层归属、技能标记、技能类型、技能备注和当前层备注。只读和可编辑状态必须视觉区分。
...
画布格子状态采用固定语义：主体区、外围区、悬停、选中、放置预览、跨格占用、承载面、被承载、上下半格叠放、可叠放、不兼容叠放、不可放置、height 派生阻塞、技能标记均有独立组合样式。不兼容叠放使用浅红提示层，但不得只靠颜色表达。

## Responsive Design & Accessibility

### Responsive Strategy

MVP 采用桌面优先 Web App。`1280px+` 使用完整三栏工作台：左侧素材库，中间尺寸驱动场景画布，右侧实例检查器，顶部工具栏与楼层/预览控制保持可见。默认 17×17 场景可以使用画布内滚动、缩放或稳定压缩，但不得让页面级横向滚动或面板遮挡核心画布。
