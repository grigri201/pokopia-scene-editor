...
该编辑器不是通用绘图工具。每次放置都必须明确绑定到一个具体实例：素材 ID、x/y 坐标、主体区或外围装饰区、建筑层、朝向和百变怪技能标记。用户应始终知道自己正在编辑的是哪个建筑层、哪个格子、哪个素材实例。
...
用户选择素材后，画布进入明确的待放置状态。悬停格子时应显示放置预览、目标坐标、区域类型、当前建筑层和技能状态。点击后，素材成为可选中的实例，用户可以立即在属性面板中确认或修改朝向、染色、技能标记和技能备注。

建筑层是一等放置上下文，不是附属选项。同一 x/y 坐标可以在不同建筑层上存在不同素材；切换建筑层时，界面必须避免用户误以为其他层同坐标内容被覆盖。
...
- 常用工具固定可达：删除、旋转、染色、俯视图和正视图等保留操作应在顶部工具栏或紧邻画布的位置可达；手动保存、导出、撤销/重做和预览覆盖开关不属于当前 MVP UI。
