...
用户还应始终看到当前 Pokemon、场景名称、当前素材、当前建筑层、选中坐标、dirty/saved 状态和双预览检查器。编辑过程不应依赖 landing page、说明页或隐藏在二级页面中的关键上下文。
...
系统必须稳定维护 5×5 主体尺寸、7×7 画布尺寸、外围扩展格数、场景名称、当前 Pokemon、建筑层、素材实例、坐标、区域类型、朝向、染色、技能标记、备注、当前编辑建筑层、当前素材、选中坐标和 dirty/saved 状态等核心数据。保存、自动保存和后续显式导出必须使用同一个 SceneDocument v1 payload，并能完整还原 Open Design 工作台的必需持久上下文。
...
- 用户可以选择当前 Pokemon、编辑场景名称，并看到 dirty/saved 状态随编辑和保存更新。
- 用户可以在右侧浮动素材栏中搜索素材、按分类筛选、只显示喜好素材，并看到当前素材和固定宽度结果计数。
- 系统能够准确识别任意坐标属于主体区还是外围装饰区。
- 用户可以在主体区和外围装饰区放置、删除、移动和替换素材。
- 用户可以创建、重命名、删除、隐藏、显示、锁定和解锁建筑层，并在左侧建筑层面板中看到 L2/L1/L0 这种高层到低层的视觉顺序，同时数据仍按 0 层到 n 层组织。
- 预览按建筑层层号从 0 层到 n 层渲染所有可见建筑层。
- 用户可以在素材放置前或放置后设置百变怪技能标记。
- 技能标记在画布、俯视图、正视图和保存数据中保持一致，技能类型使用 `树叶`、`耕地`、`储水` 词表，并以一字标签辅助识别。
- 可染色素材在格子内显示染色入口和当前颜色；非默认朝向只在 90/180/270 度时显示旋转标记，默认 0 度不额外占用画布信息层。
- 左下检查器在同一工作台内同时展示正视图和俯视图缩略预览，正视图可独立滚动。
- 保存数据重新打开后，场景名称、Decor Dex Pokemon key、画布、建筑层、当前编辑建筑层、当前素材、选中坐标、素材实例、坐标、区域类型、朝向、染色、技能标记、备注和 dirty/saved 状态能够完整还原。
- 搜索词、分类/区域/技能筛选、favorite-only 和预览显示选项不会写入 SceneDocument payload，但会在同一浏览器的 localStorage 中恢复。

MVP 验收时应使用至少 1 个完整布景方案作为验收场景，包含 Decor Dex Pokemon key、场景名称、7×7 画布、默认 3 个建筑层、当前编辑建筑层、当前素材、选中坐标、主体区素材、外围装饰区素材、至少 1 个技能标记、可染色素材、非默认朝向素材、俯视图预览、正视图预览、保存/自动保存状态和重新打开流程。验收通过标准是上述结果均可在同一 Open Design 工作台中复现，且重新打开的数据与保存前的 SceneDocument v1 payload 语义一致。
...
- 左下检查器同时展示俯视图和正视图缩略预览；俯视图显示完整 7×7、5×5 主体边界、可见建筑层和技能标记，正视图表达主体区、外围装饰区和建筑层高度关系。
- 保存/自动保存、dirty/saved 状态和本地重新打开恢复。
- SceneDocument v1 结构化序列化和恢复校验，用于保存、自动保存、恢复和后续显式导出能力；自动保存 payload 与显式导出 payload 必须完全相同。
- 重新打开保存数据后完整还原场景名称、Decor Dex Pokemon key、画布、建筑层、当前编辑建筑层、当前素材、选中坐标、素材、坐标、区域、朝向、染色、技能标记、备注和 dirty/saved 状态。
- 素材搜索词、分类/区域/技能筛选、favorite-only 和预览显示选项使用 localStorage 保存为浏览器本地 UI 偏好，不进入 SceneDocument v1 payload。
