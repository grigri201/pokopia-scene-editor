...
FR13: [Removed from MVP 2026-05-19] 系统不提供旧式通用同层叠放；Epic 11 通过 FR87-FR94 重新引入 catalog-driven 承载面与受控叠放例外。
...
FR87: Asset catalog 必须为每个可放置素材提供默认 stacking metadata，默认不可承载、不可被同层 overlap；明确 override 才能开启承载面或底垫/低高度叠放能力。

FR88: `wooden-plate`、`plate`、`party-platter` 必须被标记为 food surface，只允许 `food` category 或等价已审计食物素材作为 top item 放置在其 footprint cells 上。

FR89: 已审计的底垫、地毯、嫩芽和低高度素材可以被标记为 floor-cover 或 low-height surface；系统不得仅凭名称包含“垫”“地毯”“嫩芽”自动开放叠放。
...
NFR42: `SceneDocument v1` 仍是当前 schema；本次变更不得新增 stacking relation、surface id、z-index、parent instance id、catalog snapshot 或 `SceneDocument v2`。
...
FR13: Epic 5 - 旧式通用同层叠放从 MVP 删除；Epic 11 用 FR87-FR94 提供受控承载/叠放例外。
...
FR87: Epic 11 - Asset catalog 提供默认 stacking metadata 与明确 override。

FR88: Epic 11 - `wooden-plate`、`plate`、`party-platter` 作为 food surface 承载食物。

FR89: Epic 11 - 已审计底垫、地毯、嫩芽和低高度素材可作为 floor-cover 或 low-height surface。

FR90: Epic 11 - `scene-core` 允许且仅允许兼容 surface 的同层 overlap。

FR91: Epic 11 - 不兼容叠放返回结构化 conflict。

FR92: Epic 11 - 承载/叠放关系由 SceneDocument v1 与 asset catalog 派生，不保存关系字段。

FR93: Epic 11 - Web UI、预览和图片导出表达承载/被承载/可叠放/不兼容状态；合法叠放用上下半格显示，非法叠放用浅红冲突提示。

FR94: Epic 11 - Worker/MCP/Codex skill 复用 `scene-core` stacking helpers，导出摘要只返回 derived relation。
...
**FRs covered:** FR87, FR88, FR89, FR90, FR91, FR92, FR93, FR94, NFR41, NFR42, NFR43, NFR44.
...
**Requirements covered:** FR87, FR88, FR89, FR92, NFR41, NFR42, NFR43.
...
**Requirements covered:** FR90, FR91, FR92, FR94, NFR41, NFR42, NFR43, NFR44.
...
**Requirements covered:** FR90, FR91, FR93, NFR41, NFR43, NFR44.
...
**Requirements covered:** FR93, FR94, NFR35, NFR41, NFR42, NFR43, NFR44.
