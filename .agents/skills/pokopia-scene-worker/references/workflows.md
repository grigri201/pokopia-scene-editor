...
2. Use `structuredContent.data.summary` as the export-summary JSON, including `structuredContent.data.summary.layers[].notes` for layer notes plus `footprint`, `effectiveFootprint`, `occupiedCells`, `blockingCells`, and `footprintWarnings`.
...
2. Return asset ids, names, categories, `footprint`, and why the results fit the request.
