...
describe('asset footprint geometry', () => {
  it('derives effective footprint from rotation without changing height', () => {
    ...
    expect(asset?.footprint).toEqual({ length: 2, width: 1, height: 1 });
    expect(getEffectiveAssetFootprint(asset!.footprint, 0)).toEqual({ length: 2, width: 1, height: 1 });
    expect(getEffectiveAssetFootprint(asset!.footprint, 180)).toEqual({ length: 2, width: 1, height: 1 });
    expect(getEffectiveAssetFootprint(asset!.footprint, 90)).toEqual({ length: 1, width: 2, height: 1 });
    expect(getEffectiveAssetFootprint(asset!.footprint, 270)).toEqual({ length: 1, width: 2, height: 1 });
  });
  ...
  it('reports out-of-bounds footprint cells without clipping them', () => {
    const conflict = getFootprintBoundsConflict({
      ...
      occupiedCells: getFootprintCells({ x: 6, y: 2 }, { length: 2, width: 1 }),
